...
import CartCardMerchandise from '@/components/CartCardMerchandise';
...
const Checkout = () => {
  ...
  const [coinsToRedeem, setCoinsToRedeem] = useState(null);
  ...
  const processPayment = async (orderId) => {
    ...
    try {
      ...
      const options = {
        ...
        notes: {
          address: 'Office #719, ARG Group Road, No-9A, Sikar Road, VKIA Area, Jaipur, Rajasthan, India',
        },
        theme: {
          color: '#181818',
        },
  ...
  const applyCoins = () => {
    if (coinsToRedeem > walletData.wallet_balance) {
  ...
  return (
    <div className='w-full min-h-screen relative bg-[#f0f2f4] flex flex-col items-center'>
      ...
      {showConfetti ? <Confetti width={width} height={height} numberOfPieces={300} /> : null}
        ...
        <div className=' w-full flex h-full flex-col lg:flex-row justify-between mt-4 '>
          <div className=' flex flex-col h-full z-40 bg-white border-[0.5px] p-4 lg:p-8 border-[#c7c7c7]/60 items-start w-full lg:w-1/2'>
            <div className=' flex flex-col items-start w-full'>
              <div className=' flex flex-row w-full justify-between items-baseline'>
                ...
                {user ? null : <a className=' text-black underline text-sm'>login</a>}
              </div>
            ...
            <div className=' flex flex-col lg:flex-row justify-between items-center w-full mt-[2.5vh]'>
              <div className=' flex flex-col items-start w-full lg:w-6/12'>
                <div className=' flex flex-row justify-start items-center'>
                  ...
                  <h2 className=' text-black text-lg lg:text-xl lg:ml-2'>HydroShark Coins</h2>
                </div>
              ...
              <div className=' w-full lg:w-1/2 flex flex-row justify-between items-center mt-4 lg:mt-0 lg:flex-col lg:items-end '>
                <div className=' flex flex-row lg:w-3/12 justify-end items-center'>
                  ...
                  <p className=' text-black text-sm lg:text-lg mt-1'>{walletData?.wallet_balance}</p>
                </div>
            ...
            <div className=' flex flex-col items-start w-full mt-[5vh]'>
              <div className=' flex flex-row w-full justify-between items-baseline'>
                <p className=' text-xl text-black font-semibold'>Shipping Address</p>
              </div>
              {loading ? (
                        ...
                        >
                          <div className={` w-1/12 flex flex-col justify-center items-start`}>
                            <input
                              ...
                              checked={addressSelect.shippingAddress == address.id}
                              className=' checked:text-black'
                            />
                          </div>
            ...
            <div className=' flex flex-col items-start w-full mt-[5vh]'>
              <div className=' flex flex-row w-full justify-between items-baseline'>
                <p className=' text-xl text-black font-semibold'>Billing Address</p>
              </div>
              {loading ? (
                <div className=' flex flex-col items-center justify-center h-[10vh]  w-full'>
                  <Spinner loading={loading} size={24} color='#000000' />
            ...
            <div className=' flex flex-col items-start w-full mt-[5vh]'>
              <div className=' flex flex-row w-full justify-between items-baseline'>
                <p className=' text-xl text-black font-semibold'>Payment Method</p>
              </div>
              <div className=' flex flex-col items-center w-full justify-center bg-gray-100 mt-4 rounded-md'>
                <button
                  ...
                  className={` flex flex-row justify-between w-full py-4 px-2 border-[1px] ${
                    paymentMethod == 'razorpay' ? 'border-black' : 'border-white bg-gray-300'
                  } `}
                >
                  ...
                  <div className='w-3/12 flex flex-row  justify-end gap-x-4 '>
                    ...
                    <div className='h-[3vh] w-3/12 relative'>
                      <Image
                        src={process.env.NEXT_PUBLIC_API_URL + '/mastercardlogo.jpg'}
                        fill
                        style={{ objectFit: 'contain' }}
                      />
                    </div>
                ...
                {paymentMethod === 'razorpay' ? (
                  <div className=' flex flex-col h-[25vh] w-full justify-center items-center'>
                    ...
                    <p className=' text-black text-sm mt-2 w-10/12 lg:w-7/12 text-center'>
                      {
                        'After clicking "Proceed to pay", you will be redirected to Razorpay Secure (UPI, Cards, Wallets, NetBanking) to complete your purchase securely.'
                      }
                    </p>
                  </div>
                ) : null}
                ...
                <button
                  ...
                  className={` flex flex-row justify-between w-full py-4 px-2 border-[1px] ${
                    paymentMethod == 'cod' ? 'border-black' : 'border-white bg-gray-300'
                  } `}
                >
                  <div className=' flex flex-row items-end justify-start'>
                    <p className='text-sm text-black'>{'COD (Cash on Delivery)'}</p>
                  </div>
                </button>
              </div>
            </div>
          ...
          <div className=' flex flex-col items-start w-full lg:w-1/2 px-[5vw] py-4'>
            {loading ? (
              ...
              <div className=' flex flex-col items-start w-full'>
                <div className=' flex flex-row w-full justify-between items-baseline'>
                  <p className=' text-xl lg:text-2xl text-black font-semibold'>Cart</p>
                </div>
                ...
                <div className=' w-full flex flex-col items-start h-full overflow-y-scroll mt-4 z-20'>
                  {cartObj?.cart_items?.length > 0 ? (
                      ...
                      {cartObj?.cart_items?.map((item, index) => {
                        ...
                        return (
                          <CartCard
                            ...
                            discounted_amount={item.product_section?.discounted_amount}
                    ...
                    <div className='flex flex-col items-center justify-center h-full w-full'>
                      <p className='text-black text-xl font-semibold'>No items in cart</p>
                    </div>
                  )}
                </div>
                  ...
                  {cartObj?.coins_used > 0 && (
                    <div className='flex flex-row items-center justify-between bg-white/60 p-2 rounded-md w-full'>
                      ...
                      <button
                        ...
                        className='ml-2 text-xl text-red-600 hover:text-red-800 transition duration-200'
                      >
                        &times;
                      </button>
                    </div>
                ...
                <div className=' w-full flex flex-col items-start mt-4'>
                  ...
                  <div className=' flex flex-row w-full justify-between items-center mt-2'>
                    ...
                    <p className=' text-red-600 text-sm lg:text-lg'>
                  ...
                  {cartObj?.coins_used > 0 ? (
                    <div className=' flex flex-row w-full justify-between items-center mt-2'>
                      <p className=' text-black text-xs lg:text-sm ml-8'>Redeemed Coins</p>
                      <p className=' text-red-600 text-xs lg:text-sm'>- ₹{cartObj?.coins_used}</p>
                    </div>
                  ) : null}
                  ...
                  {parseInt(cartObj?.cart_total_price) < 450 ? (
                    <div className=' flex flex-row w-full justify-between items-center mt-2'>
                      <p className=' text-black text-sm lg:text-lg'>Shipping Charges</p>
                      <p className=' text-black text-sm lg:text-lg'>
                        ₹50
                        {/* {cartList.reduce((acc, item) => {
                      return acc + item.price;
                  ...
                  <div className=' flex flex-row justify-between items-center w-full mt-2'>
                    <p className=' text-black text-sm lg:text-lg'>HydroShark Coins</p>
