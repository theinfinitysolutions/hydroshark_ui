...
const ViewProduct = ({ id }) => {
  ...
  const { showProductModal, setShowProductModal, addToCart, cart, setCartSidebar, user } = useStore();
    ...
    <div className='bg-[#f0f2f4] w-full min-h-screen h-full relative overflow-y-scroll  flex flex-col items-start'>
        ...
        <div className=' bg-[#f0f2f4] w-full h-full lg:h-[90vh] relative overflow-y-scroll  flex flex-col items-start'>
          ...
          <div className='flex flex-col lg:flex-row justify-between items-center h-full w-full'>
            <div className=' w-full lg:w-7/12 h-[60vh] lg:h-full flex flex-col bg-[#f0f2f4] items-center justify-center relative '>
              ...
              {with3DModel.includes(selectedProduct.product_title) && show3dModel ? (
                <ReplaceScene scene={selectedProduct.product_title.toLowerCase()} orbital={true} />
              ) : (
              ...
              <div className=' absolute left-4 top-[52.5vh] lg:top-4 px-2 py-2 lg:py-4 rounded-xl bg-gray-300 flex flex-row lg:flex-col items-center  lg:items-start gap-x-6 lg:gap-x-0 gap-y-[2.5vh]'>
                {with3DModel.includes(selectedProduct.product_title) ? (
                  <a
                    ...
                    className={`w-[5vh] h-[5vh] lg:w-[5vh] lg:h-[5vh] flex flex-col items-center justify-center border-[1px] ${
                      show3dModel ? 'text-black border-black' : 'text-gray-600 border-gray-600'
                ...
                {[...selectedProduct?.product_images].map((image, index) => {
                  return (
                    <a
                      onClick={() => {
                        setCurrentImage(image?.image?.cloudfront);
                      ...
                      className={` w-[5vh] h-[5vh] lg:w-[5vh] lg:h-[5vh] flex flex-col items-center relative  justify-center cursor-pointer rounded-full border-[1px] border-white ${
                        image?.image?.cloudfront == currentImage && !show3dModel ? 'bg-white' : 'bg-black/60'
                      }`}
                    >
                      <Image
            ...
            <div className=' w-full lg:w-5/12 h-full py-8 lg:py-0 bg-[#181818] relative flex flex-col items-start pl-8 pr-[5vw] justify-center  '>
                    ...
                    return (
                      <button
                        ...
                        className={` px-4 lg:px-0 lg:w-[5vw] text-sm flex flex-col items-center border-[1px] border-white ${
                          !(selectedSection?.id == section?.id) ? 'bg-black text-white' : 'bg-white text-black'
                        } `}
                      >
                        <p className={` ${!(selectedSection?.id == section?.id) ? 'text-white' : 'text-black'} my-2`}>
              ...
              <div className=' flex flex-col lg:flex-row items-start lg:items-center lg:justify-between w-full  mt-4 lg:mt-8'>
                <div className='flex flex-row justify-start items-center text-sm gap-x-2'>
                  ...
                  <p className=' text-white text-lg	'>/ {selectedSection?.section_title} </p>
                </div>
                <div className=' flex flex-row justify-end items-center'>
                  <p className=' text-white mr-2'>{selectedSection?.hydroshark_points_on_purchase}</p>
              ...
              <div className='flex flex-row items-center justify-between gap-x-4 w-full mt-[2.5vh]'>
                ...
                >
                  {loading ? <Spinner color='#fff' size={24} loading={loading} /> : <p>{'Add to cart'}</p>}
                </button>
              </div>
              ...
              <div className=' lg:h-12 absolute  w-full left-0 bottom-2 flex flex-row justify-center lg:justify-start gap-x-4 lg:gap-x-[10%] px-2 lg:px-8'>
                <div className=' flex flex-row items-center justify-start'>
                  ...
                  <p className=' text-sm  text-white mt-1 ml-1'>{'Low Sugar'}</p>
                </div>
                <div className=' flex flex-row items-center justify-start'>
                  <IoBanOutline className=' text-white text-lg' />
                  <p className=' text-sm  text-white mt-1 ml-1'>{'No Caffeine'}</p>
                </div>
                <div className=' flex flex-row items-center justify-start'>
                  <MdOutlineHealthAndSafety className=' text-white text-lg' />
                  <p className=' text-sm  text-white mt-1 ml-1'>{'Vitamins & Minerals'}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className=' w-full flex flex-col bg-[#181818] items-center px-[7.5vw] py-[5vh]'>
        <div className=' flex flex-col items-start w-full'>
          <div className=' flex flex-col items-start w-full'>
            <p className=' text-2xl text-white font-semibold'>Ratings and Reviews</p>
            <div className=' flex flex-col items-start w-full mt-8'>
              {ratings.map((rating, index) => (
                <div key={index} className=' flex flex-row items-center justify-start w-full mb-2'>
                  <div className=' w-9/12 lg:w-[30vw] h-3 flex flex-col items-start rounded-full border-[0.5px]  border-white bg-black'>
                    <div style={{ width: `${rating.rating / 2}%` }} className={`  h-3 rounded-full bg-white`}></div>
                  </div>
          ...
          <div className=' flex flex-col items-start mt-[5vh]'>
            {comments.map((comment, index) => {
              return (
                <div key={index} className=' flex flex-col lg:flex-row w-full items-start justify-start mb-8'>
                  <div className=' w-full lg:w-[10%] flex flex-col items-start'>
                    <div className=' flex flex-row items-center justify-center'>
                      <div className=' h-8 w-8 flex flex-col rounded-full items-center bg-white justify-center'>
                        <p className=' text-lg text-black mt-1'>{comment.name[0]}</p>
                      </div>
                      <p className=' text-white text-sm ml-2 mt-1'>{comment.name}</p>
                    </div>
                    ...
                    <div className=' flex flex-row items-center mt-2 justify-start'>
                      {Array(5)
                        ...
                        .map((_, index) => {
                          return <IoMdStar key={index} className={` text-white text-lg`} />;
                        })}
                    </div>
                  </div>
                  <div className=' w-full lg:w-3/4 flex flex-col mt-2 lg:mt-0 items-start'>
                    <p className=' text-white text-sm lg:text-base'>{comment.description}</p>
