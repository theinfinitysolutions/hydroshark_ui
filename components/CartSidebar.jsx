...
export default function CartSidebar() {
  ...
  const { showAuthModal, setShowAuthModal, setShowLoading, activeCart } = useStore();
  ...
  return (
    <>
      <div style={{ zIndex: 200 }} className={`${className} ${appendClass} z-30`}>
        {loading ? (
          ...
          <div className='flex flex-col px-4 md:px-8 h-full items-center'>
            ...
            <div className='flex flex-col absolute bottom-8 w-10/12'>
              {cartList.length > 0 || cart.length > 0 ? (
                ...
                >
                  {loading ? <Spinner size={24} loading={loading} color='#ffffff' /> : 'Checkout'}
                </button>
              ) : null}
            </div>
          </div>
