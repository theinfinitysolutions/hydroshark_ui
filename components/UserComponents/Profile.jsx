...
const Profile = () => {
  ...
  return (
    <div className=' w-full flex flex-col items-start'>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className='w-full grid grid-cols-1 lg:grid-cols-3 gap-x-4'>
      ...
      <div className=' w-full flex flex-col items-end'>
        <button type='submit' className='bg-black text-white p-2 rounded-lg mt-2 px-8'>
          {loading ? <Spinner loading={loading} color='#ffffff' size={16} /> : 'Save'}
        </button>
      </div>
      ...
      <div className=' flex flex-row justify-between items-center w-full mt-[5vh]'>
        <div className=' flex flex-col items-start w-6/12'>
          <div className=' flex flex-row justify-start items-center'>
            ...
            <h2 className=' text-black text-base lg:text-xl ml-2'>HydroShark Coins</h2>
          </div>
          <p className=' text-black text-xs'>{'(You can redeem Hydroshark coins by purchasing HydroShark Gymwear)'}</p>
        </div>

        <div className=' flex flex-col items-end w-1/2'>
          <div className=' flex flex-row w-3/12 justify-end items-center'>
            <PiCoinsFill className=' text-2xl text-black' />
            <p className=' text-black text-lg mt-1'>{walletData?.wallet_balance}</p>
          </div>
      ...
      <div className=' flex flex-col items-start w-full mt-[5vh]'>
        ...
        <div className=' w-full grid grid-cols-1 lg:grid-cols-2 gap-6'>
          {addressList.map((address, index) => {
            return (
              <div key={index} className='flex flex-row w-full justify-between bg-gray-100 p-4 rounded-lg '>
