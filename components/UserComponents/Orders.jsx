...
const Orders = () => {
  ...
  const { setShowOrderDetailsModal, showOrderDetailsModal, setShowSubmitReviewModal, showSubmitReviewModal } =
    useStore();
        ...
        <div className='w-full flex flex-col items-start'>
            ...
            {next || prev ? (
              <div className=' flex flex-row items-center justify-center z-30'>
                <a
                  ...
                  className={`py-2 px-2 rounded-md cursor-pointer ${prev ? 'text-black ' : 'text-gray-400'}  `}
                  disabled={prev}
                >
                  <FaChevronLeft />
                </a>
                ...
                <a
                  ...
                  className={`${next ? 'text-black ' : 'text-gray-400'} py-2 px-2 rounded-md cursor-pointer z-20 `}
                  disabled={next}
                >
                  <FaChevronRight />
                </a>
          ...
          {orders.length == 0 ? (
            <div className=' flex flex-col items-center justify-center h-[30vh]'>
              <p className='text-lg text-black font-semibold'>No Orders Placed Yet</p>
            </div>
          ) : (
            <div id='address' className=' flex flex-col items-start max-h-[70vh] overflow-y-scroll mt-4 w-full'>
                  ...
                  >
                      ...
                      <div className=' flex flex-row w-full justify-between lg:justify-end items-center gap-x-8'>
                        {order.review?.id ? (
                          <div className=' flex flex-row justify-start gap-x-2'>
                            ...
                            <p className=' text-base text-green-400'>Review Submitted</p>
                          </div>
                        ) : (
                          <button
                            onClick={() => {
                          ...
                          >
                            ...
                            <p className=' text-base text-black'>Submit a Review</p>
                          </button>
                        )}
                    ...
                    <div className=' flex flex-row justify-between bg-gray-200 rounded-md items-center w-full py-2  px-4 '>
                      <div className=' flex flex-col items-start'>
                        ...
                        <p className=' text-sm text-black'>{order.payment.payment_method || 'N/A'}</p>
                      </div>
                      ...
                      <div className=' flex flex-col items-start'>
                        ...
                        <p className=' text-sm text-black'>{order.payment.payment_amount || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                );
