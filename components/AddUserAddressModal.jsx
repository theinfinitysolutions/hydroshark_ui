...
const AddUserAddressModal = () => {
  ...
  const createAddress = async (data) => {
    ...
    if (data.zipcode.length != 6) {
      const response = await validatePinCode(data.zipcode);
  ...
  return (
    <div className={`${isOpen ? 'fixed' : 'hidden'} z-50 inset-0 flex items-center justify-center bg-black/10`}>
          ...
          <div className=' flex flex-col items-start w-full px-4 lg:px-8'>
            ...
            >
              <div className='flex flex-col items-start  w-full mt-4  overflow-hidden'>
                ...
                />
                {errors.address_line_1 && <p className='text-red-500 text-xs mt-[4px]'>This field is required</p>}
              </div>
              <div className='flex flex-col items-start  w-full mt-4  overflow-hidden'>
                <input
                ...
                />
                {errors.address_line_2 && <p className='text-red-500 text-xs mt-[4px]'>This field is required</p>}
              </div>
              <div className='flex flex-row justify-between w-full mt-4'>
                <div className='flex flex-col items-start  w-[47.5%]  overflow-hidden'>
                  ...
                  />
                  {errors.city && <p className='text-red-500 text-xs mt-[4px]'>This field is required</p>}
                </div>
                <div className='flex flex-col items-start  w-[47.5%]  overflow-hidden'>
                  <input
                    placeholder='State'
                  ...
                  />
                  {errors.state && <p className='text-red-500 text-xs mt-[4px]'>This field is required</p>}
                </div>
              </div>
              <div className='flex flex-row justify-between w-full mt-4'>
                <div className='flex flex-col items-start  w-[47.5%]  overflow-hidden'>
                  ...
                  />
                  {errors.country && <p className='text-red-500 text-xs mt-[4px]'>This field is required</p>}
                </div>
                <div className='flex flex-col items-start  w-[47.5%] overflow-hidden'>
                  <input
                    placeholder='Zipcode'
                  ...
                  />
                  {errors.zipcode && <p className='text-red-500 text-xs mt-[4px]'>This field is required</p>}
                </div>
              </div>
              <div className=' w-full flex flex-row justify-end items-center gap-x-4 mt-4'>
                ...
                <button className=' text-white bg-black px-4 py-2 border-[1px] rounded-md border-black' type='submit'>
                  {loading ? (
                    <Spinner loading={loading} color='#fffffff' size={16} />
                  ) : (
                    <p className=' mt-1'>{mode == 'edit' ? 'Update' : 'Submit'}</p>
                  )}
                </button>
              </div>
            </form>
            ...
            <div className='flex flex-col items-start mt-4'>
              {addressList.length > 0 ? <p className=' text-xl text-black font-bold'>Address List</p> : null}
              {loading ? (
                <Spinner loading={loading} size={48} color='#000000' />
              ) : (
                <div id='address' className='flex flex-col h-[20vh] overflow-y-scroll  items-start w-full mt-4'>
                  {addressList &&
                    addressList.map((address, index) => (
                      <div key={index} className='flex flex-row justify-between bg-gray-100 p-4 rounded-lg mb-4'>
