...
export default function MobileSidebar() {
  ...
  const { sidebar, setShow, showAuthModal, setShowAuthModal, user } = useStore();
  ...
  const MenuItem = ({ icon, name, route }) => {
    ...
    return (
      <Link
        ...
        href={route}
        className={`flex flex-row items-baseline gap-1 [&>*]:my-auto  pl-6 py-2 border-b-[1px] border-b-black/10 ${colorClass}`}
      >
        <div className='text-[#181818] text-lg '>{name}</div>
      </Link>
    );
  };
