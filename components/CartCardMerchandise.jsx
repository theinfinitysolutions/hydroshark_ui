...
const CartCardMerchandise = ({ item, onQuantityChange, onDelete }) => {
  const { product_section } = item;
    ...
    <div className='w-full flex flex-col items-start p-2 lg:p-4 border-[1px] border-black mb-3'>
      <div className='flex flex-row w-full justify-between items-center'>
        <div className='flex flex-col items-start w-[15%] lg:w-[15%] h-[10vh] relative'>
          <Image
            src={item.product_section.product_primary_image.image.cloudfront}
        ...
        <div className='w-[80%] lg:w-[75%] h-full flex flex-col justify-center items-start px-4'>
          <p className='text-xs lg:text-base w-full text-black'>{item.product_section.linked_product.product_title}</p>
          ...
          <div className='flex flex-row w-full justify-start gap-x-6  items-center mt-4'>
            <div className='flex flex-row justify-between items-center gap-x-2 py-1 px-2 lg:px-4 border-[1px] border-black'>
              ...
              <p className=' text-black text-sm mt-1'>{item.quantity}</p>
              <a
                onClick={() => {
                  onQuantityChange('add');
                }}
                className=' cursor-pointer text-black text-sm'
              >
                <LuPlus />
            ...
            <div
              style={{ backgroundColor: primaryColor.product_color.color_name }}
