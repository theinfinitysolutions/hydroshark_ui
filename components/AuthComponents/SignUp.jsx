...
const inputClass = 'border-[1px] border-black  px-2 py-2 rounded-lg my-2 w-full text-black cursor-text';
...
const SignUp = ({ BackToLogin }) => {
  ...
  const onSubmit = (data) => {
    ...
    instance
      ...
      .catch((err) => {
        ...
        if (err.response && Object.keys(err.response.data.response).length > 0) {
          setError(err.response.data.response[Object.keys(err.response.data.response)[0]]);
        }
        setLoading(false);
      });
  };
  ...
  return (
    <div className='flex flex-col w-full items-center px-4'>
      <div className='text-[1.5rem] font-[500] text-black'>{showOTP ? 'Enter OTP' : 'SignUp'}</div>
      {showOTP ? (
        ...
        <div className=' flex flex-col items-center justify-center w-full'>
          <form className='flex flex-col w-full items-center mt-4' onSubmit={handleSubmit(onSubmit)}>
            ...
            <button type='submit' className='bg-black text-white px-[5vw] py-2 mt-2'>
              {loading ? <Spinner loading={loading} size={24} /> : <p className=' mt-1'>SignUp</p>}
            </button>
          </form>
        </div>
      )}
